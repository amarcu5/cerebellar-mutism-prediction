...
/** Returns mutism status (CMS=1, no CMS=0) for a given sample. */
unsigned mutismStatusHelper(float *input, float *output) {
...
int main(int argc, char **argv) {
  ...
  // mutism status
  std::vector<FannTrainData> mutism_data = StratifyTrainData(
      data_combined, 2, mutismStatusHelper);
  CrossValidation(mutism_data, [](FannTrainData &training_data,
    ...
    std::vector<FannTrainData> mutism_data = StratifyTrainData(
        training_data, 2, mutismStatusHelper);
    ...
    CrossValidation(mutism_data, [&](FannTrainData &training_data_subsample,
